...
<template>
    <div class="w-full flex flex-col rounded-lg overflow-hidden text-fa-text bg-fa-bg" :style="ColorStyle()">
        <div class="overlap-grid w-full h-20 rounded-lg overflow-hidden">
            <LoadingImage class="object-cover scale-110 blur" :src="CoverImage()" :alt="`${album.name} album cover`" />
            <div class="section-gradient opacity-60"></div>
            <div class="section-gradient-custom opacity-40" :style="`--section-gradient-col: ${album.colors.accent}`">
            </div>
            <div class="flex flex-row gap-3 items-center">
                <div class="size-20">
                    <LoadingImage class="object-cover rounded-lg" :src="CoverImage()" :alt="`${album.name} album cover`" />
                </div>
                <div class="drop-shadow-text">
                    <p class="text-xl leading-6">{{ album.name }} <span class="opacity-80 text-sm" v-if="album.release.year">{{ album.release.year }}</span></p>
                    <p class="text-base leading-5 opacity-80">{{ album.tracks.length }} songs</p>
                </div>
            </div>
        </div>
        <div class="w-full flex flex-col p-2 gap-1">
            <FeaturedTrack :playing="current[0] == album.id && current[1] == track.id"
                @play="state => emit('play', track.id, state)" :track="track" v-for="track in album.tracks" />
        </div>
    </div>
</template>