...
await API.PerformGet('/artists').then(res => {
    // Sort by name
    res.data.sort((a, b) => {
...
<template>
    <div class="w-full flex flex-col rounded-3xl bg-dark-2 p-5">
        ...
        <h3 class="text-base mb-3">Featured Artists are artists that have given us permission/we bought licenses to use
            their songs in fluXis.</h3>
        ...
        <div class="flex justify-center gap-5 flex-wrap items-start content-start" v-if="react.data">
            <RouterLink :to="'/artist/' + artist.id" v-for="artist in react.data">
                <div class="flex flex-col items-center bg-dark-3 hover:scale-105 rounded-3xl transition-transform">
                    <img class="future loadFade w-40 h-40 rounded-3xl"
                         :src="'/featured-artist/' + artist.id + '/icon.png'" :alt="artist.name" />
