...
await API.PerformGet<FeaturedArtist>('/artists/' + id)
    .then((res) => {
...
function TrackFinished(album: string, track: string) {
    if (react.current[0] != album || react.current[1] != track) return;
        ...
        <div class="w-80 min-w-80 bg-dark-3 rounded-2xl h-fit">
            <div class="size-80">
                <LoadingImage
                    ...
                    :src="'/featured-artist/' + react.artist.id + '/icon.png'"
                    alt="Artist Image"
                />
            </div>
            <div class="flex flex-row flex-wrap h-fit w-full has-[div]:p-2">
                <a
                    v-if="react.artist.youtube"
                    class="min-w-16 flex-1"
                    target="_blank"
                    :href="`https://youtube.com/${react.artist.youtube}`"
                    ><FeaturedArtistSocial class="bg-social-youtube text-social-youtube" name="YouTube"
                /></a>
                <a
                    v-if="react.artist.spotify"
                    class="min-w-16 flex-1"
                    target="_blank"
                    :href="`https://open.spotify.com/artist/${react.artist.spotify}`"
                    ><FeaturedArtistSocial class="bg-social-spotify text-social-spotify" name="Spotify"
                /></a>
                <a
                    v-if="react.artist.soundcloud"
                    class="min-w-16 flex-1"
                    target="_blank"
                    :href="`https://soundcloud.com/${react.artist.soundcloud}`"
                    ><FeaturedArtistSocial class="bg-social-soundcloud text-social-soundcloud" name="SoundCloud"
                /></a>
                <a
                    v-if="react.artist.twitter"
                    class="min-w-16 flex-1"
                    target="_blank"
                    :href="`https://twitter.com/${react.artist.twitter}`"
                    ><FeaturedArtistSocial class="bg-social-twitter text-social-twitter" name="Twitter"
                /></a>
                <RouterLink :to="`/u/${react.artist.fluxis}`" class="min-w-16 flex-1" v-if="react.artist.fluxis"
                    ><FeaturedArtistSocial
                        class="bg-primary text-primary"
                        base="https://fluxis.flux.moe/u/"
                        name="fluXis"
                        :id="'1'"
                /></RouterLink>
            </div>
            ...
            <div class="hidden">
                <div v-for="album in react.artist.albums">
                    <audio
                        ...
                        :src="`/featured-artist/${react.artist.id}/${album.id}/${track.id}/preview.ogg`"
                    ></audio>
                </div>
            </div>
        </div>
